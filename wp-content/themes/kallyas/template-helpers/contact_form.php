...
function text ( $field ) {
	...
	$field_id = str_replace ( " ", "" ,$field['zn_cf_name'] );
	
	 
	echo '<div class="control-group '.$req.'">';
		echo '<label class="control-label" for="'.$field_id.'">'.$field['zn_cf_name'].'</label>';
		echo '<div class="controls">';
			echo '<input id="'.$field_id.'" type="text" name="'.$field['zn_cf_name'].'" placeholder="'.$field['zn_cf_name'].'" class="input-xlarge '.$required.' '.$email_field.'" title="'.$field['zn_cf_name'].'" />';
		echo '</div>';
...
function textarea ( $field ) {
	...
	echo '<div class="control-group '.$req.'">';
		echo '<label class="control-label" for="'.$field_id.'">'.$field['zn_cf_name'].'</label>';
		echo '<div class="controls">';
			echo '<textarea id="'.$field_id.'" name="'.$field['zn_cf_name'].'" placeholder="'.$field['zn_cf_name'].'" class=" textarea span4 '.$required.' '.$email_field.'" title="'.$field['zn_cf_name'].'" ></textarea> ';
		echo '</div>';
	echo '</div><!-- end control group -->';
}
