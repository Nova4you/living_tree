				...
				<ul id="limited_offers">
					...
					if ($r->have_posts()) {
						...
						while ($r->have_posts()) {
							...
							echo '<li data-discount="'.$reduced.'%">';
								do_action( 'woocommerce_before_shop_loop_item_title' );
								echo '<h5><a href="'.get_permalink().'">'.get_the_title().'</a></h5>';
								echo '<h6>'.$product->sale_price.'</h6>';
							echo '</li>';
	...
	function _team_box( $options )
	{
		...
		echo '<div class="'.$element_size['sizer'].'">';
			...
			echo '<div class="team_member">';
				...
				// SOCIAL ICONS
				if ( !empty ( $options['single_team_social'] ) && is_array ( $options['single_team_social'] ) ) {
				...
				echo '<div class="tabbable">';
					...
					if ( !empty ( $options['single_vertical_tab'] ) && is_array( $options['single_vertical_tab'] ) ) {
						...
						foreach ( $options['single_vertical_tab'] as $tab ) {
							...
							// Tab Handle
							echo '<li class="'.$cls.'">';
								echo '<a href="#tabs_v2-pane'.$i.'" data-toggle="tab">';
									echo $icon;
									echo $tab['vts_tab_title'];
								echo '</a>';
							...
							// TAB CONTENT
							$content .= '<div class="tab-pane '.$cls.'" id="tabs_v2-pane'.$i.'">';
...
--------------------------------------------------------------------------------------------------*/
	...
	function _hover_box ( $options )
	{
		...
		echo '<div class="'.$element_size['sizer'].'">';
		echo '<a href="'.$options['hb_link']['url'].'" target="'.$options['hb_link']['url'].'" class="hover-box '.$options['hb_align'].' fixclear">';
				...
				<?php
					...
					while( have_posts() ){
						...
						echo '<div class="span4 post">';	
							...
							echo '<h3 class="m_title">'.get_the_title().'</h3>';
										...
										echo '<li '.$cls.'>';
											...
											echo '<div class="inner-acc">';
												...
												echo '<div class="content">';
													...
													echo '<h5 class="m_title">'.get_the_title().'</h5>';
				...
				echo '<div class="span2">';
					...
					echo '<a href="'.$options['cab_button_link']['url'].'" class="circlehover with-symbol" data-size="" data-position="top-left" data-align="right" target="'.$options['cab_button_link']['target'].'">';
						...
						if ( !empty ( $options['cab_button_image'] ) ) {
							...
							echo '<span class="symbol"><img src="'.THEME_DIR.'/images/ok.png" alt=""></span>';
						}
					echo '</a>';
				echo '</div>';
			}
				...
				<div class="hg-portfolio-sortable">
					...
					<ul id="thumbs" class="fixclear">
						...
						<?php if ( have_posts() ): while ( have_posts() ): the_post(); ?>
							...
							<li class="item <?php echo strtolower(str_replace(' ','_',strip_tags ( get_the_term_list( $post->ID, 'project_category', '', '', '' ) ) ) );?> even" data-date="<?php the_time('Y/m/l'); ?>">
		...
		if ( !empty ( $options['steps_single2'] ) && is_array ( $options['steps_single2'] ) ) {
			...
			foreach ( $options['steps_single2'] as $step ) {
				...
				if ( $step['stp_single_ok'] == 'yes' ) {
					$ok = 'ok';
					$image = '<img src="'.THEME_DIR.'/images/ok.png" alt="">';
				}
...
--------------------------------------------------------------------------------------------------*/
	...
	?>
		<div id="slideshow" class="<?php echo $style; ?> portfolio_devices">
			<div class="bgback"></div>
			<div data-images="<?php echo IMAGES_URL; ?>/" id="sparkles"></div>
...
--------------------------------------------------------------------------------------------------*/
	function _header_module($options)
	{
		...
		if ( !empty ( $options['hm_header_height'] ) ) {			$height='style="height:'.$options['hm_header_height'].'px"';		}
						...
						foreach ( $options['single_iosslider'] as $slide ) {
							...
							echo '<div class = "item">';
								...
								echo '<div class="caption '.$c_style.' '.$c_pos.'">';
									...
									{
										echo '<a class="more" href="'.$slide['io_slide_link']['url'].'" target="'.$slide['io_slide_link']['target'].'"><img src="'.THEME_DIR.'/sliders/iosslider/arr01.png" alt=""></a>';
									}
				...
				"markers": [
					{
						...
						"icon": "<?php echo THEME_DIR;?>/images/map_pin_1.png"	// Pin icon
					}
				]
			};
		<?php } ?>
                                	...
                                	<h4 style="text-align:center"><?php _e("Create <strong>your account</strong> now",THEMENAME);?></h4>
										<form id="register_form_static" name="login_form" method="post" class="zn_form_login" action="<?php echo site_url('wp-login.php?action=register', 'login_post') ?>">
	                                    	<div>
	                                            ...
	                                            <input name="user_login" type="text" id="reg-username" value="">
	                                        </div>
	                                        <div>
	                                        	<label for="reg-email">Email</label>
	                                            <input name="user_email" type="email" id="reg-email" value="">
	                                        </div>
                        ...
                        <div class="row">
							...
							?>
								...
								<div class="ud_counter">
									...
									<?php echo '<span class="till_lauch"><img src="'.THEME_DIR.'/images/rocket.png"></span>'; ?>
                                </div><!-- end counter -->
