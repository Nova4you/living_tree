...
function zn_shortcode_skills( $atts, $content = null ) {
	...
	$return = '<div id="skills_diagram">';
		...
		$return .= '<script type="text/javascript" src="'.THEME_DIR.'/addons/raphael_diagram/init.js"></script>';
...
function zn_shortcode_table( $atts, $content = null ) {
	...
	return str_replace ( '<table' ,'<table class="table '.$type.'"', $content );
...
function zn_shortcode_buttons( $atts, $content=null ) {
	// [button style="btn-primary" size="" block="false"] BUTTON TEXT [/button]
	extract(shortcode_atts(array(
		"style" => '',
		"size" => '',
		"block" => ''
	...
	$button = ' <button class="btn '.$size.' '.$style.' '.$block.'" type="button">'.$content.'</button>';
	return $button;
