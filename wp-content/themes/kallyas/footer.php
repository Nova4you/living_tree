		...
		<div class="container">
			...
			<?php	
				if ( isset ( $data['footer_row1_widget_positions'] ) && !empty ( $data['footer_row1_widget_positions'] ) ) {
					...
					for ($i = 1; $i <= $number_of_columns; $i++) {
						echo '<div class="span'.$columns_array[$number_of_columns][0][$i-1].'">';
							if ( !function_exists('dynamic_sidebar') || !dynamic_sidebar('Footer row 1 - widget '.$i.'') ) : endif; 
						echo '</div>';
					}
				...
				if ( isset ( $data['footer_row2_widget_positions'] ) && !empty ( $data['footer_row2_widget_positions'] ) ) {
					...
					for ($i = 1; $i <= $number_of_columns; $i++) {
						echo '<div class="span'.$columns_array[$number_of_columns][0][$i-1].'">';
							if ( !function_exists('dynamic_sidebar') || !dynamic_sidebar('Footer row 2 - widget '.$i.'') ) : endif; 
						echo '</div>';
					}
			...
			<div class="row">
				<div class="span12">
					...
					<?php
						if ( isset( $data['footer_social_icons'] ) && is_array( $data['footer_social_icons'] ) ) {
