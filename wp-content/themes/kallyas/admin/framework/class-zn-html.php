...
***************************************************/
	...
	function zn_show_options($options) 
	{
	...
	function zn_page_start() {
		...
		$output .= $this->zn_show_menu($zn_admin_menu);
		$output .= '</div>';
		...
		$output .= '<div class="h_info"><a href="#">Version '.$my_theme->get( 'Version' ).'</a></div>';
...
***************************************************/
	...
	function media ( $value ){
		...
		$output .= '<input class="logo_upload_input" id="'.$value['id'].'" type="text" size="36" name="'.$value['id'].'" value="'.$value['std'].'" />';
		$output .= '<input class="zn_upload_image_button" type="button" value="Upload Image" />';
		
		if( isset ( $value['std'] ) && !empty( $value['std'] ) )
		{
			$output .= '<div class="zn-image-holder"><a class="zn-remove-image" href="#">remove</a><img alt="" src="'.$value['std'].'"></div>';
		}
		else
		{
