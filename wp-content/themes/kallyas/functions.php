		...
		if ( empty( $data['head_show_logo'] ) || (!empty( $data['head_show_logo'] ) && $data['head_show_logo'] == 'yes') ) {
			if ( is_front_page() ) {
				if ( isset ( $data['logo_upload'] ) && !empty ( $data['logo_upload'] ) ) {
					$logo = '<h1 id="logo"><a href="'.home_url().'"><img src="'.$data['logo_upload'].'" alt="'.get_bloginfo('description').'" /></a></h1>';
				}
				else {
				...
				if ( isset ( $data['logo_upload'] ) && !empty ( $data['logo_upload'] ) ) {
					$logo = '<h3 id="logo"><a href="'.home_url().'"><img src="'.$data['logo_upload'].'" alt="'.get_bloginfo('description').'" /></a></h3>';
				}
				else {
...
if ( ! function_exists( 'zn_favicon' ) ) {
	function zn_favicon(){ 
		...
		if(isset($data['custom_favicon']) && !empty($data['custom_favicon'])){ 
			$favicon = '<link rel="shortcut icon" href="'.$data['custom_favicon'].'" type="image/vnd.microsoft.icon"/>';
			$favicon = '<link rel="icon" href="'.$data['custom_favicon'].'" type="image/x-ico"/>';
		} else {
			$favicon = '<link rel="shortcut icon" href="'.get_bloginfo('template_directory').'/images/favicons/favicon.png" type="image/vnd.microsoft.icon"/>';
			$favicon = '<link rel="icon" href="'.get_bloginfo('template_directory').'/images/favicons/favicon.png" type="image/x-ico"/>';
		}
	...
	add_action('admin_head','zn_favicon'); 
...
if ( ! function_exists( 'zn_add_styles' ) ) {
	function zn_add_styles()
	{
		...
		if (!is_admin())
		{
			...
			wp_enqueue_style('Lato_default', 'http://fonts.googleapis.com/css?family=Lato:300,400,700,900&amp;v1&mp;subset=latin,latin-ext', false, 1.0, 'screen');
			wp_enqueue_style('Open+Sans_default', 'http://fonts.googleapis.com/css?family=Open+Sans:400,400italic,700&amp;v1&mp;subset=latin,latin-ext', false, 1.0, 'screen');
		...
		if ( $post ) {
			...
			foreach ($metas as $options ) 
			{
				...
				if  ( $options['dynamic_element_type'] == '_c_form' ) {
					...
					$contact_form = array ( 'zn_testimonial_slider' =>
							"
							(function($){
...
");
				...
				if  ( $options['dynamic_element_type'] == '_partners_logos' ) {
					...
					$partners_logos = array ( 'zn_partners_logo' =>
							"jQuery(window).load(function(){
							// ** partners carousel
							jQuery('#partners_carousel').carouFredSel({
								responsive: true,
								scroll: 1,
								auto: false,
								items: {
				...
				if  ( $options['dynamic_element_type'] == '_static4' ) {
					...
					wp_enqueue_script('nivo_slider', get_template_directory_uri() . '/js/mapmarker.jquery.js',array('jquery'),'1.3',true); 
				}
...
if ( ! function_exists( 'zn_add_script' ) ) {
	function zn_add_script()
	{
		...
		if (!is_admin()){
			...
			wp_enqueue_script('modernizr', 'http://cdnjs.cloudflare.com/ajax/libs/modernizr/2.6.1/modernizr.min.js',array('jquery'),'1.3',true);
...
--------------------------------------------------------------------------------------------------*/
	...
	if ( function_exists('register_sidebar') ) {
		register_sidebar(array(
			...
			'id' => 'defaultsidebar',
			'description' => esc_html__('This is the default sidebar. You can choose from the theme\'s options page where the widgets from this sidebar will be shown.'),
	...
	if ( function_exists('register_sidebar') ) {
		register_sidebar(array(
			...
			'description' => esc_html__('This is the sidebar for the hidden panel in the header. You can choose from the theme\'s options page where the widgets from this sidebar will be shown.'),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widgettitle title">',
			'after_title' => '</h3>'
...
--------------------------------------------------------------------------------------------------*/
	...
	if ( isset ( $data['footer_row1_widget_positions'] ) && !empty ( $data['footer_row1_widget_positions'] ) ) {
		...
		if ( function_exists('register_sidebars') ) {	
			register_sidebars( key( json_decode ( stripslashes ( $data['footer_row1_widget_positions'] ) ) ),$args);
		}
	...
	if ( isset ( $data['footer_row2_widget_positions'] ) && !empty ( $data['footer_row2_widget_positions'] ) ) {
		$args = array(
			...
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title' => '<h4 class="widgettitle title m_title">',
			'after_title' => '</h3>' ); 
			
		if ( function_exists('register_sidebars') ) {	
			register_sidebars( key( json_decode ( stripslashes ( $data['footer_row2_widget_positions'] ) ) ),$args);
		}
		...
		foreach ($data['sidebar_generator'] as $sidebar) {
			if ( $sidebar['sidebar_name'] ) {
				register_sidebar(array(
					...
					'before_widget' => '<div id="%1$s" class="widget %2$s">',
					'after_widget'  => '</div>',
					'before_title' => '<h4 class="widgettitle title">',
					'after_title' => '</h4>'
				));
			}
		}
...
if ( ! function_exists( 'woocommerce_header_add_to_cart_fragment' ) ) {	
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		?>
		<span class="cart_details"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?> <?php _e("Total of",THEMENAME);?> <?php echo $woocommerce->cart->get_cart_total(); ?> <a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'woothemes'); ?>" class="checkout"><?php _e("Checkout",THEMENAME);?> <span class="icon-chevron-right"></span></a></span>
		<?php
		...
		elseif ( is_home() ) {
			...
			elseif ( is_post_type_archive('product') && get_option('page_on_front') !== woocommerce_get_page_id('shop') ) {
					...
					if ( is_search() ) {
						...
						echo '<li typeof="v:Breadcrumb"><a rel="v:url" property="v:title" href="' . get_post_type_archive_link('product') . '">' . $_name . '</a></li><li>'. __('Search results for &ldquo;', 'woocommerce') . get_search_query() . '</li>';
	...
	<div class="login_register_stuff hide"><!-- Login/Register Modal forms - hidded by default to be opened through modal -->
		...
		<div id="forgot_panel">
			<div class="inner-container forgot-panel">
				...
				<form id="forgot_form" name="login_form" method="post" class="zn_form_login" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>">
					<p>
...
add_action('init', 'zn_coming_soon_page');
				...
				if  ( $options['dynamic_element_type'] == '_c_form' ) {
					if ( isset ($_POST['cform_submit']) ) {
						...
						foreach ( $options['zn_cf_fields'] as $field ) {
							...
							// Compoese the mail
							if ( isset ( $_POST[$field['zn_cf_name']] ) ) { 
							
								$body .= $field['zn_cf_name'] .' : '.$_POST[$field['zn_cf_name']] ."<br />" ;
								if ( $field['zn_cf_f_email'] ) {
									$email = $_POST[$field['zn_cf_name']];
								}
								if ( $field['zn_cf_f_name'] ) {
									$name = $_POST[$field['zn_cf_name']];
								}
							}
...
if ( ! function_exists( 'zn_login_text' ) ) {	
	function zn_login_text() {
		...
		if ( is_user_logged_in() ) {
			...
			echo '<ul class="topnav navRight"><li><a href="#login_panel" data-rel="prettyPhoto[login_panel]">LOGIN</a></li></ul>';
		}
	}
}	
		...
		if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
			...
			if ( $data['woo_show_cart'] ) {
				...
				<ul class="topnav navLeft">
					<li class="drop">
						<a id="mycartbtn" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'woothemes'); ?>">
							<?php _e( "MY CART",THEMENAME);?>
						</a>
						<div class="pPanel">
							<div class="inner">	<span class="cart_details"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?>, <?php _e("Total of",THEMENAME);?> <?php echo $woocommerce->cart->get_cart_total(); ?> <a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'woothemes'); ?>" class="checkout"><?php _e("Checkout",THEMENAME);?> <span class="icon-chevron-right"></span>
								</a>
								</span>
							</div>
						</div>
...
if ( ! function_exists( 'zn_wpml_language_switcher' ) ) {	
	function zn_wpml_language_switcher () {
		...
		if ( $data['head_show_flags'] ) {
			...
			echo '<li class="languages drop"><a href="#"><span class="icon-globe icon-white"></span> LANGUAGES</a>';
